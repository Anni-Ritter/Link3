...
namespace Link
{
    public class FirstThreadRecieve
    {

        private Semaphore _sendSemaphore;
        private Semaphore _receiveSemaphore;
        private BitArray _receivedMessage;
		private PostToFirstSendWT _post;

		public FirstThreadRecieve(ref Semaphore sendSemaphore, ref Semaphore receiveSemaphore)
		{
				...
				switch (BitConverter.ToInt32(StaticFunction.BitArrayToByteArray(item.Status), 0))
				{
					...
					case 4: //RD
						...
						if (StaticFunction.isFile)
							StaticFunction.DeserializeFile("2 поток");
						else
							StaticFunction.DeserializeMessage("2 поток");
